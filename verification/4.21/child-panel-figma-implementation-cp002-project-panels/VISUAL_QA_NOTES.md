# Visual QA Notes

## Checked

- Project top menu opens the Project Management host state.
- Create Project panel renders command draft preview-only data.
- Init Project Dirty State panel renders dirty field metadata.
- Standardization Preview panel renders profile, policy, and evidence-plan metadata.
- Compact Project menu open screenshot is nonblank.
- ContextEnvelope, Main Cockpit, Operation Panel Host, Inspector, and status bar remain visible.

## Notes

- The renderer is still a non-authoritative UX/client test surface.
- Project child panel buttons produce local preview or blocked-display behavior only.
- No direct baseline approval, direct canonical mutation, completion, production readiness, continuity activation, deploy, or final PASS claim is exposed.
