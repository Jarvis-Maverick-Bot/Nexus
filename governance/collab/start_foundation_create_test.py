"""
Start V2.0 Foundation Create — test sender.

Command: "Start V2.0 Foundation Create"
Command intent: start_foundation_delivery
Workflow: v2_0 / stage: foundation_create

This tests Layer C binding: whether the command semantic layer
can drive a real business-path action through the collab mechanism.

Expected:
1. Nova sends start_foundation_create to Jarvis
2. Jarvis daemon validates, creates collab, updates state
3. Jarvis returns ACK to Nova
4. Collab state shows: status=open, pending_action=awaiting_foundation_draft
"""
...
async def main():
    ...
    # Message contract for "Start V2.0 Foundation Create"
    envelope = {
    ...
    try:
        ...
        print("\nVerify state: python -c \"import json; print(json.dumps(json.load(open('D:\\\\Projects\\\\Nexus\\\\governance\\\\data\\\\collab_state.json')).get('" + collab_id + "', {}), indent=2))\"")
